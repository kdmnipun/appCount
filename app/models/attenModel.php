...
/*       $sql = "SELECT 
  ...
  public function selectMemberForAtn($tbl_preatn,$data){
    return $this->db->insert($tbl_preatn,$data);
  } 
  ...
  public function checkAtnFromPreTbl($tbl_preatn){
    $sql = "SELECT * FROM $tbl_preatn ORDER BY pre_id DESC"; 
    return $this->db->select($sql);
  }
  ...
  public function atnList($tbl_preatn,$tbl_member){
       ...
       $sql ="SELECT $tbl_preatn.*,$tbl_member.id,name,gender,member,reg,batch,phone FROM $tbl_preatn
              INNER JOIN $tbl_member
