...
<div id="page-wrapper">
    <div class="container-fluid">
        ...
        <div class="row" style="margin-bottom: 50px;">
            <div class="col-lg-12">
                <form id="addAttndForm" class="attnfo keepFormDat" action="" method="post" data-keep-form-data-clear-on-submit="yes"  autocomplete="off">
                          ...
                          <?php }?>
                            ...
                            <td>
                                ...
                                <select name="attend[<?php echo $value['id'];?>]" id="attendF" class="form-control">
                                  <option id="none<%=none++%>" noneId="<?php echo $value['id'];?>" value="">Select</option>

                                  <option 
                                  <?php
                                  foreach ($checkAtn as $key => $val) {
                                  ?>
                                  <?php if($val['memberid'] == $value['id']){?>
                                  selected
                                  <?php }?>
                                  <?php }?>
                                  id="present<%=present++%>" getId="<?php echo $value['id'];?>" class="attend select"  value="present">Present</option>

                                  <option  id="absent<%=absent++%>" absentId="<?php echo $value['id'];?>" value="absent">Absent</option>
...
$(function(){
 ...
 $(document).ready(function(){  
      $('[id^="present"]').click(function(){  
              var ids =  $(this).attr('getId');   
           //alert(ids);  
           $.ajax({  
                url:"<?php echo BASE_URL; ?>/Attendance/selectMemberForAtn",  
                method:"POST",  
                data:{ids:ids},  
                success:function(data){  
                //toastr.success('Member Added successfully', 'Success Alert',{timeOut: 1000,progressBar: true}); 
                }  
             });  
         });  
    });
  ...
  $(document).ready(function(){  
      $('[id^="absent"]').click(function(){  
             var ids =  $(this).attr('absentId');   
           //alert(ids);  
           $.ajax({  
  ...
  $(document).ready(function(){  
   ...
   $('#reset').click(function(e){
    ...
    if(confirm('Are you sure?'))
       {
           $.ajax({  
                url:"<?php echo BASE_URL; ?>/Attendance/clearTable",
                method:"POST",  
                data:data,  
                success:function(data){  
